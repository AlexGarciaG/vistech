...
class Node:
    def __init__(self):
        ...
        self.address_master = int(rospy.get_param("~address_master", "128"))
        self.address_slave = int(rospy.get_param("~address_slave", "129"))
        if self.address_master > 0x87 or self.address_master < 0x80:
            #rospy.logfatal("Master Address out of range")
            rospy.signal_shutdown("Master Address out of range")
        if self.address_slave > 0x87 or self.address_slave < 0x80:
            #rospy.logfatal("Slave 1 Address out of range")
            rospy.signal_shutdown("Slave 1 Address out of range")
        ...
        #rospy.logdebug("Master address %d", self.address_master)
        #rospy.logdebug("Slave address %d", self.address_slave)
    ...
    def start_comunication(self):
        while True:
            ...
            try:
                with self.mutex:
                    version = self.roboclaw.ReadVersion(self.address_master)
            except Exception as e:
            ...
            try:
                with self.mutex:
                    self.roboclaw.SpeedM1M2(self.address_master, 0, 0)
                    self.roboclaw.ResetEncoders(self.address_master)
                    self.roboclaw.SpeedM1M2(self.address_slave, 0, 0)
                    self.roboclaw.ResetEncoders(self.address_slave)
            except OSError as e:
                    #rospy.logerr("Could not stop motors")
                    #rospy.logdebug(e)
                    continue
            break
    ...
    def check_vitals(self, stat):
        try:
            with self.mutex:
                status = self.roboclaw.ReadError(self.address_master)[1]
        except OSError as e:
        ...
        try:
            with self.mutex:
                stat.add("Main Batt V:", float(self.roboclaw.ReadMainBatteryVoltage(self.address_master)[1] / 10))
                stat.add("Logic Batt V:", float(self.roboclaw.ReadLogicBatteryVoltage(self.address_master)[1] / 10))
                stat.add("Temp1 C:", float(self.roboclaw.ReadTemp(self.address_master)[1] / 10))
                stat.add("Temp2 C:", float(self.roboclaw.ReadTemp2(self.address_master)[1] / 10))
        except OSError as e:
            #rospy.logwarn("Diagnostics OSError: %d", e.errno)
            #rospy.logdebug(e)
            pass
